...
class FBertaDualSenseInputDevice final : public IInputDevice
{
	public:
		...
		void AppendConnectedDevices(TArray<FBertaDualSenseDeviceInfo>& Out, const FPlatformUserId User = PLATFORMUSERID_NONE) const { for(const TPair<SDL_JoystickID,FConnectedDualSense>& Pair:ConnectedDevices)if(!User.IsValid()||Pair.Value.PlatformUserId==User){ FBertaDualSenseDeviceInfo Info; BuildDeviceInfo(Pair.Value,Info); Out.Add(MoveTemp(Info)); } }
		TArray<FBertaDualSenseDeviceInfo> GetConnectedDevicesForControllerId(int32 ControllerId) const { TArray<FBertaDualSenseDeviceInfo> Out; const FPlatformUserId User=GetPlatformUserForControllerId(ControllerId);if(User.IsValid())AppendConnectedDevices(Out,User);return Out; }
...
void FBertaDualSenseModule::GetConnectedDevices(TArray<FBertaDualSenseDeviceInfo>& OutDevices) const { OutDevices.Reset(); for(const TWeakPtr<FBertaDualSenseInputDevice>& Weak:CreatedInputDevices)if(const TSharedPtr<FBertaDualSenseInputDevice> Device=Weak.Pin())Device->AppendConnectedDevices(OutDevices); OutDevices.Sort([](const FBertaDualSenseDeviceInfo&A,const FBertaDualSenseDeviceInfo&B){return A.Device.DeviceId<B.Device.DeviceId;}); }
void FBertaDualSenseModule::GetDevicesForControllerId(int32 ControllerId,TArray<FBertaDualSenseDeviceInfo>& OutDevices) const { OutDevices.Reset();for(const TWeakPtr<FBertaDualSenseInputDevice>& Weak:CreatedInputDevices)if(const TSharedPtr<FBertaDualSenseInputDevice> Device=Weak.Pin())OutDevices.Append(Device->GetConnectedDevicesForControllerId(ControllerId));OutDevices.Sort([](const FBertaDualSenseDeviceInfo&A,const FBertaDualSenseDeviceInfo&B){return A.Device.DeviceId<B.Device.DeviceId;});}
bool FBertaDualSenseModule::GetDeviceInfo(const FBertaDualSenseDeviceHandle& Device,FBertaDualSenseDeviceInfo& OutInfo) const { for(const TWeakPtr<FBertaDualSenseInputDevice>& Weak:CreatedInputDevices)if(const TSharedPtr<FBertaDualSenseInputDevice> Input=Weak.Pin())if(Input->GetDeviceInfo(Device,OutInfo))return true;OutInfo={};return false; }
...
bool FBertaDualSenseModule::SetLightColorForDevice(const FBertaDualSenseDeviceHandle& Device,FColor Color) { for(const TWeakPtr<FBertaDualSenseInputDevice>& Weak:CreatedInputDevices)if(const TSharedPtr<FBertaDualSenseInputDevice> Input=Weak.Pin())if(Input->IsDeviceConnected(Device))return Input->SetLightColorForDevice(Device,Color);return false; }
