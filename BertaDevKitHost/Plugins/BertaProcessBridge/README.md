...
In a future manually authorized Editor session, run `BertaProcessBridge.Lifecycle.BlockedInputOwnerShutdown` in the Automation Test window on Win64. It launches a child that does not consume stdin, queues 512 KiB, then deinitializes the owner. Confirm that the test returns promptly, input is rejected after shutdown, and no child remains. Also repeat with a child that reads stdin to confirm normal delivery and completion. This test is compiled by the host Editor build but is not run by the build.
