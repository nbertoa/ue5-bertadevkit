...
FBertaProjectSetupReport FBertaProjectSetup::Audit()
{
	...
	bool bScriptStack = false; const FString DefaultEngine = GetDefaultEngineConfigPath(); GConfig->GetBool(ScriptStackSection, ScriptStackKey, bScriptStack, DefaultEngine); AddValue(Report, TEXT("Script Stack on Warnings"), BoolText(bScriptStack), TEXT("True"), true);
...
void FBertaProjectSetup::ApplyWithConfirmation()
{
	...
	const FString DefaultEngine = GetDefaultEngineConfigPath(); GConfig->SetBool(ScriptStackSection, ScriptStackKey, true, DefaultEngine); GConfig->Flush(false, DefaultEngine); if (UObject* LiveCoding = FindSettingsClassDefault(TEXT("/Script/LiveCoding.LiveCodingSettings"))) WriteBoolProperty(LiveCoding, TEXT("bAutomaticallyCompileNewClasses"), false);
