...
- `UBertaGSCAbilityQueueInputBridgeComponent` observes the native ASC failure/end delegates and reconciles them on the next frame. With `UGSCAbilitySystemComponent`, its public queue state exposes the native handling and the bridge does not forward again; with another ASC implementation, the bridge can supply the missing queue calls. It deliberately skips ambiguous same-frame state rather than risk duplicate activation, so run the version-specific manual checks below.
